# -*- coding: utf-8 -*-
"""
Created on Tue Feb  5 23:31:56 2019
...
def get_badrep_df():
    ...
    for i in range(len(data['categories']['official'])):
        badrep.append(data['categories']['official'][i]['address'])
    badrep_df = pd.DataFrame(index=np.array(badrep))
    #csv_final=df_final.to_csv
    return badrep_df

def get_delegators_status_df(delegators_df,badrep,votes_threshold):
...
if __name__ == '__main__':
    ...
    status=get_delegators_status_df(delegators,badrep_df,votes_threshold)
